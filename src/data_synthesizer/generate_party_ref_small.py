#!/usr/bin/env python3
"""
Small Party Reference Generator
Generates a small sample of party_ref.csv from the small source files
"""
...
def load_small_source_data():
    """Load data from the small source files"""
    print("Loading small source data...")
    
    # Load transaction data
    trnx_df = pd.read_csv('data/sample_trnx.csv')
    print(f"Loaded {len(trnx_df)} transaction records")
    
    # Load Orbis data
    orbis_df = pd.read_csv('data/sample_orbis.csv')
    print(f"Loaded {len(orbis_df)} Orbis records")
    
    # Load WorldCheck data
    wc_df = pd.read_csv('data/sample_wc.csv')
...
def save_party_ref_sample(parties: List[Dict], filename: str = 'data/party_ref_sample.csv'):
    """Save a small sample of party reference data to CSV"""
    print(f"Saving sample of {min(5, len(parties))} parties to {filename}...")
    
    # Take first 5 parties or all if less than 5
    sample_parties = parties[:5]
    
    df = pd.DataFrame(sample_parties)
    df.to_csv(filename, index=False)
    
    print(f"Party reference sample saved to {filename}")
    print("\nSample data:")
    print(df.to_string(index=False))

def main():
    """Main function to generate small party reference sample"""
    print("Starting small party reference generation...")
    
    # Load small source data
    trnx_df, orbis_df, wc_df = load_small_source_data()
    ...
    # Save small sample
    save_party_ref_sample(all_parties)
    
    print("\nSmall party reference generation completed!")
    print(f"Generated {len(all_parties)} total parties")
    print("Check data/party_ref_sample.csv for the 5-row sample")
