#!/usr/bin/env python3
"""
Full-Scale Sample Data Generator for AML Entity Resolution
...
def generate_orbis(customers):
    ...
    for i in range(ORBIS_COUNT):
        if i % 10000 == 0:
            print(f"Generated {i} Orbis records...")
        ...
        orbis_record = {
            ...
            'incorporation_date': fake.date_between(start_date='-20y', end_date='now').strftime('%Y-%m-%d'),
            'status': 'Active',
            ...
            'last_updated': fake.date_between(start_date='-1y', end_date='now').strftime('%Y-%m-%d')
        }
...
def generate_worldcheck(customers):
    ...
    for i in range(WORLDCHECK_COUNT):
        if i % 10000 == 0:
            print(f"Generated {i} WorldCheck records...")
        ...
        worldcheck_record = {
            ...
            'listing_date': fake.date_between(start_date='-5y', end_date='now').strftime('%Y-%m-%d'),
            'last_updated': fake.date_between(start_date='-1y', end_date='now').strftime('%Y-%m-%d'),
            'status': 'Active',
...
def generate_transactions(customers, orbis_records, worldcheck_records):
    ...
    for i, customer in enumerate(customers):
        if i % 10000 == 0:
            print(f"Generated transactions for {i} customers...")
    ...
    for i in range(remaining_transactions):
        if i % 10000 == 0:
            print(f"Generated {i} additional transactions...")
...
def main():
    ...
    print("Starting full-scale sample data generation...")
    print(f"Target volumes: {CUSTOMER_COUNT:,} customers, {ORBIS_COUNT:,} Orbis, {WORLDCHECK_COUNT:,} WorldCheck, {TRANSACTION_COUNT:,} transactions")
    ...
    print("Full-scale sample data generation completed!")
