#!/usr/bin/env python3
"""
Test version of Sample Data Generator for AML Entity Resolution
Generates smaller test datasets to verify functionality
"""
...
# Test configuration (smaller numbers)
CUSTOMER_COUNT = 1000
ORBIS_COUNT = 200
WORLDCHECK_COUNT = 200
...
def generate_transactions(customers):
    """Generate transactions with all customers appearing"""
    ...
    for i, customer in enumerate(customers):
        ...
        for j in range(num_transactions):
            transaction = generate_single_transaction(customer, i, j)
            transactions.append(transaction)
    
    # Generate remaining transactions
    for i in range(remaining_transactions):
        customer = random.choice(customers)
        transaction = generate_single_transaction(customer, len(transactions), 0)
        transactions.append(transaction)
    
    return transactions

def generate_single_transaction(customer, transaction_num, sub_num):
...
def main():
    ...
    print("Starting test sample data generation...")
    
    # Generate customers first
    customers = generate_customers()
    save_to_csv(customers, 'data/sample_customer_test.csv', 
                ['id', 'name', 'email', 'phone', 'address', 'source_system'])
    
    # Generate Orbis with customer overlap
    orbis_records = generate_orbis(customers)
    save_to_csv(orbis_records, 'data/sample_orbis_test.csv',
    ...
    save_to_csv(worldcheck_records, 'data/sample_wc_test.csv',
    ...
    save_to_csv(transactions, 'data/sample_trnx_test.csv',
    ...
    print("Test sample data generation completed!")
